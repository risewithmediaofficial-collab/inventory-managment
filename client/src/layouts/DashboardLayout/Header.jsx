...
export default function Header() {
  ...
  return (
    <>
      <header className="h-[60px] bg-white border-b border-gray-100 flex items-center justify-between px-6 flex-shrink-0 sticky top-0 z-30">
        <div className="flex items-center gap-4">
          <button onClick={() => dispatch(toggleMobileSidebar())} className="lg:hidden btn-icon btn-ghost">
            <Menu size={20} />
          </button>
          {/* Search trigger */}
          <button
            onClick={() => dispatch(setGlobalSearch(true))}
            className="flex items-center gap-2 px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-400 hover:bg-gray-100 transition-colors w-64"
          >
            <Search size={15} />
            <span className="flex-1 text-left">Search...</span>
            <kbd className="flex items-center gap-1 text-xs text-gray-300 font-mono">
              <Command size={11} />K
            </kbd>
          </button>
        </div>
