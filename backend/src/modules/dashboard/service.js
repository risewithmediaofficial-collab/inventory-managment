...
export const getDashboardStats = async (companyId) => {
  ...
  const [
    ...
    // 5. All-time sales total
    Sale.aggregate([
      { $match: { companyId: cId, type: 'invoice' } },
      { $group: { _id: null, count: { $sum: 1 }, total: { $sum: '$totalAmount' } } },
    ]),

    // 6. All-time purchases total
    Purchase.aggregate([
      { $match: { companyId: cId, type: 'purchase_invoice' } },
      { $group: { _id: null, count: { $sum: 1 }, total: { $sum: '$totalAmount' } } },
    ]),
    ...
    // 9. Supplier count
    Supplier.countDocuments({ companyId: cId }),

    // 10. Purchase order count
    Purchase.countDocuments({ companyId: cId }),

    // 11. Pending receivables (what customers owe)
    Sale.aggregate([
      { $match: { companyId: cId, type: 'invoice', paymentStatus: { $in: ['unpaid', 'partial'] } } },
      { $group: { _id: null, total: { $sum: '$dueAmount' }, count: { $sum: 1 } } },
    ]),

    // 12. Pending payables (what we owe suppliers)
    Purchase.aggregate([
    ...
    // 16. Last 7 days sales for trend chart
    Sale.aggregate([
      {
        $match: {
          companyId: cId,
          type: 'invoice',
          saleDate: { $gte: new Date(Date.now() - 7 * 86400000) },
        },
      },
      {
        $group: {
  ...
  const allSalesData = allTimeSales[0] || { count: 0, total: 0 };
  const allPurchasesData = allTimePurchases[0] || { count: 0, total: 0 };
  ...
  return {
    kpis: {
      // Matching frontend DashboardPage.jsx keys:
      todaySales: todaySalesData.total,
      todayInvoices: todaySalesData.count,
      totalRevenue: allSalesData.total,
      totalProducts: pStats.total,
      lowStockCount: pStats.lowStock,
      totalCustomers: customerCount,
      totalPurchases: allPurchasesData.total,
      totalPurchaseOrders: poCount,
      totalSuppliers: supplierCount,
      totalOutstanding: pendingPayments[0]?.total || 0,
      pendingDues: pendingPayments[0]?.total || 0,

      // Additional standard keys for api compatibility:
