...
export const createPurchase = async (data, companyId, userId) => {
  ...
  // ── 4. Update stock for received invoices ────────────────────────────────
  if (data.type === 'purchase_invoice' && data.status === 'received') {
    try {
      ...
      console.error('Purchase stock update error (non-fatal):', stockErr.message);
    }
  }

  // ── 5. Update supplier balance ───────────────────────────────────────────
  if (dueAmount > 0) {
    await Supplier.findByIdAndUpdate(data.supplier, { $inc: { currentBalance: dueAmount } });
  }

  emitToCompany(companyId, 'purchase:created', {
    purchase: { _id: purchase._id, purchaseNumber, totalAmount },
  });
