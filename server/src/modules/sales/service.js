...
export const createSale = async (data, companyId, userId) => {
  ...
  try {
    // Validate customer
    const customer = await Customer.findOne({ _id: data.customer, companyId });
    ...
    if (data.type === 'invoice' && data.status !== 'draft') {
      await updateStockForSale(sale, companyId, userId, session, 'decrease');
    ...
    if (dueAmount > 0) {
      await Customer.findByIdAndUpdate(data.customer, { $inc: { currentBalance: dueAmount } }, { session });
    }
...
const updateStockForSale = async (sale, companyId, userId, session, direction) => {
